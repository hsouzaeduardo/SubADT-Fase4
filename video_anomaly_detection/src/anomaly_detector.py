"""
Módulo de Detecção de Anomalias em Vídeos de Vigilância 
- Remove uso de time.time() e usa exclusivamente `timestamp` (tempo do vídeo).
- Corrige lógica de "stopped_since" para sempre usar timestamp.
- Mantém a estrutura original para facilitar integração com seu pipeline atual.
"""
...
class AnomalyDetector:
    """Detector de anomalias em vídeos de vigilância (tempo corrigido)"""
    ...
    def detect(
        ...
        # Atualizar estados (agora usando timestamp do vídeo)
        ...
        for track in tracks:
            ...
            if self._check_sudden_movement(track_id):
                frame_anomalies.append(
                    self._create_anomaly('MOVIMENTO_SUBITO', track, frame_number, timestamp)
                )

            # 2) Velocidade anormal
            if self._check_high_speed(track):
                frame_anomalies.append(
                    self._create_anomaly('VELOCIDADE_ANORMAL', track, frame_number, timestamp)
                )

            # 3) Parada prolongada (tempo do vídeo)
            if self._check_prolonged_stop(track_id, activities.get(track_id), timestamp):
                frame_anomalies.append(
                    self._create_anomaly('PARADA_PROLONGADA', track, frame_number, timestamp)
                )

            # 4) Movimento reverso
            if self._check_reverse_movement(track_id):
                frame_anomalies.append(
                    self._create_anomaly('MOVIMENTO_REVERSO', track, frame_number, timestamp)
                )

            # 5) Objeto abandonado (tempo do vídeo)
            if self._check_abandoned_object(track, activities.get(track_id), timestamp):
                frame_anomalies.append(
                    self._create_anomaly('OBJETO_ABANDONADO', track, frame_number, timestamp)
                )

        # 6) Aglomeração
        frame_anomalies.extend(self._check_crowding(tracks, frame_number, timestamp))
    ...
    def _check_sudden_movement(self, track_id: int) -> bool:
        ...
        return max(accelerations) > self.thresholds['sudden_acceleration']
    ...
    def _check_prolonged_stop(self, track_id: int, activity: Optional[str], timestamp: float) -> bool:
        """Detecta parada prolongada (baseado no tempo do vídeo)"""
        if activity != 'PARADO':
            return False
        ...
        stopped_since = state.get('stopped_since')
        if stopped_since is None:
            return False

        stopped_duration = timestamp - stopped_since
    ...
    def _check_reverse_movement(self, track_id: int) -> bool:
        ...
        initial = state.get('initial_position')
        ...
        max_distance = state.get('max_distance', 0.0)
        thr = self.thresholds['return_threshold']

        return (distance < thr) and (max_distance > thr * 2)

    def _check_abandoned_object(self, track: Dict, activity: Optional[str], timestamp: float) -> bool:
        """Detecta objeto abandonado (não é pessoa e está parado)"""
        # Apenas objetos, não pessoas
    ...
    def _check_crowding(self, tracks: List[Dict], frame_number: int, timestamp: float) -> List[Dict]:
        """Detecta aglomerações"""
        anomalies = []

        people = [t for t in tracks if t.get('class_id') == 0]
    ...
    # -------------------------
    # State update (tempo corrigido)
    # -------------------------
    def _update_track_states(self, tracks: List[Dict], timestamp: float):
        """Atualiza estado de cada track (usando tempo do vídeo)"""
        ...
        for track in tracks:
            ...
            state['max_distance'] = max(state.get('max_distance', 0.0), float(distance))

            # Verificar se está parado (mantém a regra original em pixels/frame)
            speed = np.linalg.norm(velocity)
            if speed < 2.0:
        ...
        for tid in to_remove:
            last_seen = self.track_states[tid].get('last_seen', timestamp)
            if (timestamp - last_seen) > 5.0:
    ...
    # Helpers / outputs
    # -------------------------
    def _create_anomaly(self, anomaly_type: str, track: Dict, frame_number: int, timestamp: float) -> Dict:
